...
contains
  ...
  subroutine SetServices(gcomp, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize InitializeP1 failed")) return

    call NUOPC_CompSpecialize(gcomp, specLabel=label_Advertise, &
      specRoutine=Advertise, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize Advertise failed")) return

    call NUOPC_CompSpecialize(gcomp, specLabel=label_RealizeExternal, &
      specRoutine=Realize, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize Realize failed")) return

    call NUOPC_CompSpecialize(gcomp, specLabel=label_DataInitialize, &
      specRoutine=DataInitialize, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize DataInitialize failed")) return

    call NUOPC_CompSpecialize(gcomp, specLabel=label_Advance, &
      specRoutine=ModelAdvance, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize Advance failed")) return

    call NUOPC_CompSpecialize(gcomp, specLabel=label_Finalize, &
      specRoutine=Finalize, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Specialize Finalize failed")) return
  ...
  subroutine Advertise(gcomp, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg="NUOPC_ModelGet failed")) return

    call NUOPC_Advertise(importState, StandardName="air_temperature", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Advertise air_temperature failed")) return
    call NUOPC_Advertise(importState, StandardName="air_pressure", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Advertise air_pressure failed")) return
    call NUOPC_Advertise(exportState, StandardName="specific_humidity", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Advertise specific_humidity failed")) return
    call NUOPC_Advertise(exportState, StandardName="precipitation_rate", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Advertise precipitation_rate failed")) return
  ...
  subroutine Realize(gcomp, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg="GetInternalState failed")) return

    call NUOPC_ModelGet(gcomp, importState=importState, exportState=exportState, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="NUOPC_ModelGet failed")) return

    call NUOPC_Realize(importState, grid=state%grid, fieldName="air_temperature", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Realize air_temperature failed")) return
    call NUOPC_Realize(importState, grid=state%grid, fieldName="air_pressure", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Realize air_pressure failed")) return
    call NUOPC_Realize(exportState, grid=state%grid, fieldName="specific_humidity", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Realize specific_humidity failed")) return
    call NUOPC_Realize(exportState, grid=state%grid, fieldName="precipitation_rate", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg="Realize precipitation_rate failed")) return
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg="GridCompGet failed")) return
